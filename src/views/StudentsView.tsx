...
export const StudentsView: React.FC<StudentsViewProps> = ({
  ...
  const fetchStudents = useCallback(async () => {
    ...
    try {
      const res = await studentService.getAll({
        ...
        major: deptFilter !== 'ALL' ? deptFilter : undefined,
      });
      let arr = [];
      let total = 0;
      let pages = 1;
      if (Array.isArray(res)) {
        arr = res;
        total = res.length;
      } else if (typeof res === 'object' && Array.isArray((res as any).content)) {
        arr = (res as any).content;
        total = (res as any).totalElements ?? arr.length;
        pages = (res as any).totalPages ?? 1;
      } else if (typeof res === 'object' && Array.isArray((res as any).data)) {
        arr = (res as any).data;
        total = arr.length;
      }
      // Support _page metadata set by apiClient
      if ((res as any)?._page) {
        total = (res as any)._page.totalElements ?? total;
        pages = (res as any)._page.totalPages ?? pages;
      }
  ...
  // Server-side filtering: reset to page 0 on filter/search change
  ...
  const filtered = students; // server already filtered
  ...
  return (
    <PageContainer>
      ...
      {isLoading ? (
        ...
        <div className="rounded-xl border border-slate-200/90 bg-white shadow-2xs overflow-hidden">
          <div className="px-3.5 py-2.5 border-b border-slate-100 flex items-center justify-between text-xs text-slate-500">
            <span>Danh sách sinh viên: <strong className="text-slate-800 font-semibold">{totalElements}</strong></span>
            <span className="text-slate-400">Trang {currentPage + 1}/{totalPages}</span>
          </div>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between gap-3 rounded-xl border border-slate-200/90 bg-white px-4 py-2.5 shadow-2xs">
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              className="rounded-lg border border-slate-200 bg-white px-2 py-1 text-[11px] font-medium text-slate-600 hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
            >«</button>
            <button
              type="button"
              disabled={currentPage === 0}
              onClick={() => setCurrentPage(p => Math.max(0, p - 1))}
              className="rounded-lg border border-slate-200 bg-white px-2.5 py-1 text-[11px] font-medium text-slate-600 hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
            >‹ Trước</button>
