...
export const studentService = {
  getAll: (params?: StudentQueryParams) => {
    ...
    if (params?.major && params.major !== 'ALL') query.append('major', params.major);
    const queryString = query.toString();
    return api.get<any>(`/api/students${queryString ? `?${queryString}` : ''}`);
  },
...
export interface MentorDetailDTO extends MentorDTO {
  phoneNumber?: string;
  avatarUrl?: string;
...
export const mentorService = {
  getAll: (params?: MentorQueryParams) => {
    const query = new URLSearchParams();
    if (params?.page !== undefined) query.append('page', String(params.page));
    if (params?.size !== undefined) query.append('size', String(params.size));
    if (params?.sortBy) query.append('sortBy', params.sortBy);
    ...
    const queryString = query.toString();
    return api.get<any>(`/api/mentors${queryString ? `?${queryString}` : ''}`);
  },
  getById: (id: number) => api.get<MentorDTO>(`/api/mentors/${id}`),
  getDetail: (id: number) => api.get<MentorDetailDTO>(`/api/mentors/${id}/detail`),
...
export const assessmentResultService = {
  getAll: (page = 0, size = 10) => api.get<any>(`/api/assessment-results?page=${page}&size=${size}`),
  getById: (id: number) => api.get<AssessmentResultDTO>(`/api/assessment-results/${id}`),
  getByAssignmentAndRound: (assignmentId: number, roundId: number) => 
    api.get<any>(`/api/assessment-results/assignment/${assignmentId}/round/${roundId}`),
  getByPhase: (phaseId: number) => 
    api.get<any>(`/api/assessment-results/phase/${phaseId}`),
};
